...
public class CircuitBoard {
	...
	public CircuitBoard(String filename) throws FileNotFoundException {
		...
		try {
		ROWS = line.nextInt(); //replace with initialization statements using values from file
		COLS = line.nextInt();
		}
		catch (InputMismatchException e) {
			throw new InvalidFileFormatException("Invalid type in constructor");
		}
		...
		try {
			for (int i = 0; i < ROWS; i++) {
				...
				for (int j = 0; j < COLS; j++) {
					tempChar = line.next().charAt(0);
					if (ALLOWED_CHARS.contains(String.valueOf(tempChar))) {
						...
						if (tempChar == START) {
							if (!startFound) {
								...
								line.close();
								throw new InvalidFileFormatException("Only one starting point is allowed");
							}
						}
							...
							if (!endFound) {
								...
								line.close();
								throw new InvalidFileFormatException("Only one end point is allowed");
							}
						}
						...
						line.close();
						throw new InvalidFileFormatException("File contains invalid character");
						
					}
				}
				if (line.hasNext()) {
					throw new InvalidFileFormatException("Invalid Dimensions");
				}
			}
			if (fileScan.hasNext()) {
				throw new InvalidFileFormatException("Invalid Dimensions");
		...
		if (!startFound||!endFound) {
			throw new InvalidFileFormatException("File does not contain a start point or end point");
		}
